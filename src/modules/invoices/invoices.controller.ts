...
export class InvoicesController {
  ...
  @ApiOperation({
    ...
    description:
      "Obtiene un listado de órdenes de compra aprobadas que pueden ser facturadas",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Registra una nueva factura para una orden de compra. Máximo 3 facturas por orden.",
  })
  @ApiResponse({ status: 201, description: "Factura creada exitosamente" })
  @ApiResponse({
    status: 400,
    description: "Datos inválidos o límite de facturas excedido",
  })
  @ApiResponse({ status: 404, description: "Orden de compra no encontrada" })
  async create(
    @GetUser() user: User,
  ...
  @ApiOperation({
    ...
    description:
      "Actualiza los datos de una factura. No se puede editar si ya fue enviada a contabilidad.",
  })
  ...
  @ApiResponse({
    status: 403,
    description: "Factura ya enviada a contabilidad",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Elimina una factura. No se puede eliminar si ya fue enviada a contabilidad.",
  })
  @ApiParam({ name: "id", type: Number })
  ...
  @ApiOperation({
    ...
    description:
      "Marca todas las facturas de una orden como enviadas a contabilidad. Solo si la suma de facturas = total de OC.",
  })
  @ApiParam({ name: "purchaseOrderId", type: Number })
  @ApiResponse({ status: 200, description: "Facturas enviadas exitosamente" })
  @ApiResponse({
    status: 400,
    description: "Las facturas no completan el total de la orden",
  })
  @ApiResponse({ status: 404, description: "Orden de compra no encontrada" })
  ...
  @ApiOperation({
    ...
    description:
      "Obtiene las órdenes de compra con facturas enviadas a contabilidad pero aún no marcadas como recibidas",
  })
  @ApiQuery({ name: "page", required: false, type: Number })
  @ApiQuery({ name: "limit", required: false, type: Number })
  @ApiResponse({ status: 200, description: "Lista obtenida exitosamente" })
  async getInvoicesPendingAccountingReception(
    ...
    return this.invoicesService.getInvoicesPendingAccountingReception(
      page,
      limit,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      "Obtiene las órdenes de compra con facturas que ya fueron marcadas como recibidas por contabilidad",
  })
  @ApiQuery({ name: "page", required: false, type: Number })
  @ApiQuery({ name: "limit", required: false, type: Number })
  @ApiResponse({ status: 200, description: "Lista obtenida exitosamente" })
  ...
  @ApiOperation({
    ...
    description:
      "Marca todas las facturas de una orden como recibidas por contabilidad. Este es el paso final del proceso de facturación.",
  })
  @ApiParam({ name: "purchaseOrderId", type: Number })
  @ApiResponse({
    status: 200,
    description: "Facturas marcadas como recibidas exitosamente",
  })
  @ApiResponse({
    status: 400,
    description: "Las facturas no han sido enviadas a contabilidad",
  })
  @ApiResponse({ status: 404, description: "Orden de compra no encontrada" })
