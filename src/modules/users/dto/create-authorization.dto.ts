...
export class CreateAuthorizationDto {
  ...
  @ApiProperty({
    description:
      "Tipo de autorización: revision (nivel 1), autorizacion (nivel intermedio), aprobacion (nivel final)",
    example: "revision",
  ...
  @IsIn(["revision", "autorizacion", "aprobacion"], {
    message:
      "El tipo de autorización debe ser: revision, autorizacion o aprobacion",
  })
  tipoAutorizacion: "revision" | "autorizacion" | "aprobacion";

  @ApiPropertyOptional({
    description:
      "ID de la gestión/módulo específico (2 = Compras). Si no se especifica, aplica a todos.",
    example: 2,
  })
  @IsOptional()
...
export class BulkAuthorizationDto {
  ...
  @ApiProperty({
    description:
      "Lista de IDs de usuarios que serán autorizados (subordinados)",
    example: [12, 13, 14],
    type: [Number],
  })
  @IsNumber({}, { each: true })
