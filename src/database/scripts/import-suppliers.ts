...
async function importSuppliers() {
  ...
  try {
    ...
    const csvPath =
      process.argv[2] || path.join(process.cwd(), "suppliers.csv");
    ...
    if (!fs.existsSync(csvPath)) {
      ...
      console.log("\nOpciones:");
      console.log(
        "  --update    Actualizar proveedores existentes (por NIT/CC)",
      );
      console.log("\nFormato CSV requerido:");
      console.log("  name;nit_cc;phone;address;city;email;contact_person");
      console.log("\nEjemplo:");
      console.log(
        '  npx ts-node src/database/scripts/import-suppliers.ts "./proveedores.csv"',
      );
      process.exit(1);
    }
    ...
    if (updateExisting) {
      console.log(
        "⚠️  Modo actualización activado: se actualizarán proveedores existentes",
      );
    }
    ...
    const nameIdx = getColumnIndex([
      "name",
      "nombre",
      "razon_social",
      "razon social",
    ]);
    const nitIdx = getColumnIndex([
      "nit_cc",
      "nit",
      "cc",
      "nit/cc",
    ...
    const contactIdx = getColumnIndex([
      "contact_person",
      "contacto",
      "persona_contacto",
      "representante",
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const contactPerson =
        contactIdx !== -1 ? parts[contactIdx]?.trim() : undefined;
    ...
    console.log(
      `   ⏭️  Proveedores omitidos:    ${stats.suppliersSkipped} (ya existían)`,
    );
