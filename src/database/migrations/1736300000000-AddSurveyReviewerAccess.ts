...
export class AddSurveyReviewerAccess1736300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS survey_reviewer_access (
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_survey_reviewer_access_user ON survey_reviewer_access(user_id);
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_survey_reviewer_access_company ON survey_reviewer_access(company_id);
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_survey_reviewer_access_project ON survey_reviewer_access(project_id);
    `);

    await queryRunner.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS idx_survey_reviewer_access_unique
